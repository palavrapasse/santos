...
func CreateAspiradorClients(serverAddress string) []as.Client {
	patternLayout := as.PatternLayout(fmt.Sprintf("[%s] [SANTOS %s] %s %s %s.%s:%s : %s", as.LevelPattern, serverAddress, as.DatePattern, as.TimePattern, as.FileNamePattern, as.MethodPattern, as.LinePattern, as.MessagePattern))
